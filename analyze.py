...
VariationCount = recordclass('VariationCount', ['yes', 'no', 'unmarked'])
...
# ambiguities, and the third number is the number of unmarked
# occurences.
variations = defaultdict(lambda: defaultdict(lambda: VariationCount(0, 0, 0)))
...
with open(filename, 'r') as f:
    ...
    for line in f:
        ...
        if match:
            ...
            # have a lemma found yet. Then check if we are on the line
            # that tells us if the variation was legitimate or not.
            ...
            # start with a 'y' or 'n' then this is a new variation
            # occurence.
            elif line[0] == '\t':
                new_variation = True
...
for header, lemmas in variations.items():
    ...
    for lemma, counts in lemmas.items():
        print('{}, {}'.format(*lemma))
        print('\tyes = {}, no = {}, unmarked = {}'.format(*counts))
